...
st.title("🧑‍💼 Recruiter Resume Auto‑Fill App (Bulk – Fixed)")
st.write("Upload multiple resumes or biodata files. Names & scores are extracted reliably.")
...
def extract_name(text):
    ...
    # Resume format: first clean name‑like line
    lines = text.split("\n")
    for line in lines:
        line = line.strip()
        if (
...
def extract_score(text):
    cgpa = re.search(r"(\d+(?:\.\d+)?)\s*(?:CGPA|GPA)", text, re.I)
